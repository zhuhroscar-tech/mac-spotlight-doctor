...
def query_lsof(target: str, *, runner: Runner = default_runner) -> List[Blocker]:
    cmd = ["lsof", "+D", target]
    result = run_command(cmd, runner=runner, timeout=12.0)

    output = result.stdout or ""
    if "not a file system" in (result.stderr or "") and not output.strip():
        # fallback: query with direct target; this often works for plain directories too.
...
def render_report(report: Dict[str, object]) -> str:
    lines = [
        ...
        f"device: {report['disk_info'].get('Device Identifier', 'unknown')}",
        f"mount_point: {report['disk_info'].get('Mount Point', 'unknown')}",
    ]

    blockers = report["blockers"]
    if not blockers:
        lines.append("status: not busy (no open handles found for this path)")
    else:
    ...
    if blockers:
        ...
        if spotlight_like:
            lines.append("suggestion: spotlight appears to hold files; try '--spotlight off' temporarily")
